    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        ...
        for t in range(T - 1):
            ...
            if t > 0:
                ...
                mode_acq: torch.Tensor = torch.tensor([[0, 1]], device=device).expand(
                    B, -1
                )
                input_acq: torch.Tensor = torch.cat(
                    [x_t_minus_1, a_t_minus_1, mode_acq], dim=1
                )
            ...
            mode_pred: torch.Tensor = torch.tensor([[1, 0]], device=device).expand(
                B, -1
            )
            input_pred: torch.Tensor = torch.cat([x_t, a_t, mode_pred], dim=1)
